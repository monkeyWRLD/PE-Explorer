...
struct MemoryEditor
{
    ...
    void DrawWindow(const char* title, void* mem_data, binary* bin, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        Sizes s;
        ...
        if (ImGui::Begin(title, &Open, ImGuiWindowFlags_NoScrollbar))
        {
            ...
            DrawContents(mem_data, mem_size, bin, base_display_addr);
    ...
    void DrawContents(void* mem_data_void, size_t mem_size, binary* bin, size_t base_display_addr = 0x0000)
    {
            ...
            for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
            {
                ...
                for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
                {
                    ...
                    else
                    {
                        ...
                        auto rva = (base_display_addr + addr);
                        ImColor col = { 48, 47, 44, 255 }; // default color (gray)

                        for (auto i = 0; i < bin->sections.size(); i++)
                        {
                            if (bin->headers.size() > i)
                            {
                                auto header = &bin->headers[i];

                                if (rva >= header->start_address && rva < header->end_address)
                                {
                                    col = header->colour;
                                    break;
                                }
                            ...
                            if (rva >= section->start_address && rva < section->end_address)
                            {
                                col = section->colour;
                                break;
                            }
                        ...
                        if (OptShowHexII)
                        {
                            if ((b >= 32 && b < 128))
                            {
                                ImGui::TextColored(col, ".%c", b);
                            }
                            else if (b == 0xFF && OptGreyOutZeroes)
                            {
                                auto grayed_out = static_cast<ImVec4>(col);
                                grayed_out.w /= 2;

                                ImGui::TextColored(static_cast<ImColor>(grayed_out), "## ");
                            }
                            else if (b == 0x00)
                                ImGui::Text("   ");
                            else
                        ...
                        else
                        {
                            if (b == 0 && OptGreyOutZeroes)
                            {
                                ...
                                ImGui::TextColored(static_cast<ImColor>(grayed_out), "00 ");
                            }
                            else
                            {
                                ImGui::TextColored(col, format_byte_space, b);
